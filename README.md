...
* 如需手机测试，要先退出开发环境，然后执行`npm run prod:build`，这个指令会把当前环境设置为生产环境并执行构建，在生产环境下构建项目，会进行代码压缩，缩小代码体积后才能正常的预览
* 如需发布体验版或者正式版小程序，也要执行`npm run prod:build`这个指令才能正常发布。
...
* 优先使用react hooks风格代码，目前只有在需要用到第三方插件的时候，不得不使用react类组件或者小程序原生代码
